...
public final class LoremIpsum {
     ...
     * @param amount The amount of words to generate.
     * @param offset How many words to skip before selecting the first one.
     * @return a String containing the words separated by space.
     */
    public static String getWords(int amount, int offset) {
        if (amount <= 0) return "";
        StringBuilder lorem = new StringBuilder();
        int word = offset % WORDS_ARRAY_LENGTH;

        String firstWord = LOREM_IPSUM_WORDS[word % WORDS_ARRAY_LENGTH];
        String casedFirstWord = firstWord.substring(0,1).toUpperCase() + firstWord.substring(1);
        lorem.append(casedFirstWord);
        lorem.append(' ');

        for(int i = word + 1; i < word + amount; ++i) {
    ...
    public static String getParagraphs(int amount, int paragraphLength) {
        ...
        for(int i = 0; i < amount; ++i) {
            lorem.append(getWords(paragraphLength));
            if (i < amount - 1) {
                lorem.append("\n\n");
            }
        }
